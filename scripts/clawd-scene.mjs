...
const PROPS = {
  ...
  calendar: (cv) => { cv.rect(PX(-2), PY(8), CELL * 4.5, CELL * 4, hx("#FAF6EC")); cv.rect(PX(-2), PY(8), CELL * 4.5, CELL, TER); for (let r = 0; r < 2; r++) for (let c = 0; c < 3; c++) cv.rect(PX(-1.4 + c * 1.4), PY(9.5 + r * 1.2), CELL * 0.8, CELL * 0.8, hx("#D8C49A")); },
  alarmclock: (cv) => { const x = PX(-1), y = PY(9.5); cv.odisc(x, y, CELL * 1.8, hx("#FAF6EC"), STEELD); cv.disc(x - CELL * 1.4, y - CELL * 1.4, CELL * 0.6, STEELD); cv.disc(x + CELL * 1.4, y - CELL * 1.4, CELL * 0.6, STEELD); cv.line(x, y, x, y - CELL, SKIN_BLUE.e, CELL * 0.3); cv.line(x, y, x + CELL * 0.7, y, SKIN_BLUE.e, CELL * 0.3); },
